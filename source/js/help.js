...
function fetchAllArticles() {
    showWindowProgress();
    $.getJSON(articlesRootURL + "/metadata.json", function(metadata){
    ...
    $.each(results, function(i, rslt) {
        ...
        '<div class="column is-full">'+
            '<a class="tag help-tag is-medium" onclick="loadArticle(\''+resultSlug+'\')">'+
                ...
                '<span>'+resultTitle+'</span>'+ resultTagToShow +
            '</a>'+
        '</div>';

        if (resultsToAppend.indexOf(srCard) === -1) {
