...
// returns null, or the year as string
...
// returns null or a an array of months.
...
// takes in a string like "summer 2019", returns an array range of exif dates or null;
...
// returns null, or the two-digit month as string.
...
// returns null, or the day as string.
...
// returns null or an array of timerange [09:00, 16:00] etc.
...
// takes a string, returns a 24H two-digit string like 22:30
function isItATime(string) {
  ...
  if (elems.length === 3) {
    if (["p","pm","a","am"].indexOf(elems[2]) > -1 && (parseInt(elems[0]) < 24) && (parseInt(elems[1]) < 60)) {
      hour = parseInt(elems[0]); if (hour < 10) { hour = "0" + hour; }
      min = parseInt(elems[1]); if (min < 10) { min = "0" + min; }
      time = hour + ":" + min;
    }
...
// takes a string, returns an EXIF compatible date string. 
// if this has a day, use it exactly, if it has a year & month only then use it as a range.
...
// returns "year", "season", "month", "day", "time of day", "time" or -null-
...
function getSyntacticReferences(searchTerm, reference, callback) {
  callback = callback || noop;
  var targetStartDate, targetEndDate;
  var timeResults, timeRange, timeOfDay, comboMonth, comboYear, comboSeason, comboTimesOfDay;
  var originalRef = reference;
    ...
    if (findOne(elements,["ago","back"]) && typeof timeago === "number" && !isNaN(timeago)) {
      // we'll use the current date to compute this - whatever the ago value is. 
        ...
        if (targetStartDate && targetEndDate) {
          reference = reference.where('date', '>=', targetStartDate).where('date', '<=', targetEndDate);
      ...
      if (reference === originalRef || elements.indexOf("year") > -1) {
        var lastYear = (thisYear - 1) + "";
        reference = reference.where('year', '==', lastYear); 
...
function combineWithMonth(elements,reference,understood) {
  if (findOne(elements,monthsShort) || findOne(elements,monthsLong)) {
    ...
    if (month) {
      reference = reference.where('month', '==', month);
...
function combineWithYear(elements,reference,understood) {
  ...
  elements.forEach(function(elem) {
    if (isItAYear(elem)) { year = elem; }
  });

  if (year) { 
    reference = reference.where('year', '==', year); 
...
function combineWithTimesOfDay(elements,reference,understood) {
  if (findOne(elements,timesOfDay)) {
    ...
    if (timeRange[0] !== "not-a-time") {
      reference = reference.where('time', '>=', timeRange[0]).where('time', '<=', timeRange[1]);
...
function syntacticSearch(searchTerm, successCallback, fallbackToTitleSearchCallback) {
  var referencesToSearch = [];
  var referencesToQuery = [];

  allFoldersArray = allFoldersArray || [];
  allFoldersArray.forEach(function(fid) {
    // for all photos in folders
    referencesToSearch.push(homeRef.doc(fid).collection("photos")); 
  });

  // First a dry run with homeRef. This is to make sure the search is indeed syntactic. 
  // So we don't make tons of requests.
  // if it's syntactic, result will have an array of references.
  // if not syntactic, result won't have a reference. 
  
  getSyntacticReferences(searchTerm, homeRef, function(homeRefResult, understood) {
    if (homeRefResult) {
      
      // this adds the homeRef query reference
      referencesToQuery.push(homeRefResult);

      // this adds other folders' query references
      referencesToSearch.forEach(function(refToSearch){
        getSyntacticReferences(searchTerm, refToSearch, function(ref, understood) {
          referencesToQuery.push(ref);
        });
      });

      breadcrumb("[SEARCH] Running syntactic search");
      querySyntacticSearch(searchTerm, referencesToQuery, function(results){
        console.log("[SEARCH] Syntactic search complete!");
        successCallback(results, understood);
      });

    } else {
      breadcrumb("[SEARCH] Running title search");
      fallbackToTitleSearchCallback(searchTerm);
    }
  });

}

function querySyntacticSearch (searchTerm, refs, callback) {
  searchTerm = searchTerm || "";
  refs = refs || null;
  var promisesToKeep = [];
  var syntacticSearchResults = {};
  refs.forEach(function(ref){
    promisesToKeep.push(
      ref.get().then(function(querySnapshot) {
        querySnapshot.forEach(function(doc) {
          // doc.data() is never undefined for query doc snapshots
          syntacticSearchResults[doc.id] = syntacticSearchResults[doc.id] || {};
          syntacticSearchResults[doc.id] = doc.data();
          var fid =doc.ref.parent.parent.id;
          syntacticSearchResults[doc.id].fid = fid;
          syntacticSearchResults[doc.id].fname = titlesIndex.folders[fid];
          syntacticSearchResults[doc.id].name = titlesIndex.photos[doc.id];
        });
  ...
  return Promise.all(promisesToKeep).then(function(){
    callback(syntacticSearchResults);
  });
}

// gotSyntacticSearchResult is in photos.js (since that's where it will be used)