...
var itemsDisallowedInTableCells = ["header", "blockquote", "code-block", "list", "list-item", "indent", "formula", "video", "image", "", "divider", "hr"];
    ...
    opsAtIndex.forEach(function(op) {
        if (op.attributes) {
            ...
            if (op.attributes.crypteetable) {
                tableid = op.attributes.crypteetable;
                tableIDsToSelect[tableid] = true;          
            }

            if (op.attributes.crypteetabledata) {
                tableid = op.attributes.crypteetabledata.tableid;
                tableIDsToSelect[tableid] = true;          
            }
        }
    });

    if (range.length > 0 && Object.keys(tableIDsToSelect).length > 0) {
        var lowestTableStartIndex = 0;
        var highestTableEndIndex = 0;
        Object.keys(tableIDsToSelect).forEach(function(tableid){
...
function populateTablePicker() {
    ...
    if (winW <= 480) {
        $(".table-picker-cell[c=8]").remove();
        $(".table-picker-cell[c=7]").remove();
        $(".table-picker-cell[r=8]").remove();
        $(".table-picker-cell[r=7]").remove();
    }
}
...
$('.ql-new-table').click(function () {
    showFlyingModal("new-table-modal"); 
});
...
$("#table-contextual-button").on('click', function(event) {
    ...
    // 30 (cell height) + 4px for optical coolness 
    var topPos = selectedCellCoords.top + 34 + "px";
    if (selectedCellCoords.top + 34 + 280 >= winH) {
...
function showTableContextualDropdown() {
    $("#table-dropdown").addClass("visible");
}

function hideTableContextualDropdown() {
    $("#table-dropdown").removeClass("visible");
}
...
// We use _proto.insertAt in both the table and cell blots to filter out newline / return characters to ensure the grid won't break,
...
function toggleTableWarning(warn) {
    ...
    if (warn) {
        table.addClass("warn");
        $("#table-contextual-button").addClass("warn");
    } else {
        table.removeClass("warn");
        $("#table-contextual-button").removeClass("warn");
    }
